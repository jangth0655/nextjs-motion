...
const PostSlider = ({
  ...
  const backPage = (back: boolean) => {
    setPostPage && back === true
  ...
  return (
    <div className="text-gray-700 mt-8 relative ">
      <div className="flex items-center justify-center pb-4  h-80">
        ...
        <div className="absolute w-[80%] h-full border-x-[1px] px-4 rounded-lg border-x-orange-200">
          {userPost?.map((post) => (
            <div className="space-y-5" key={post.id}>
              ...
              <FavWithCommentCount favCount={favCount} postCount={postCount} />
            </div>
          ))}
        </div>
      </div>
