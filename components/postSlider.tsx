...
const PostSlider = ({
  ...
  const onPage = () => {
    ...
    if (userPost && postCount) {
      ...
      isBack
        ? setPage((prev) => (prev === 0 ? totalPost : page - 1))
        : setPage((prev) => (prev === maxPage ? 0 : page + 1));
    }
    console.log(isBack);
  };

  const toggleIsBack = (back: boolean) => {
    back === true ? setIsBack(true) : setIsBack(false);
  };
  ...
  return (
    <div className="text-gray-700 mt-8  relative ">
      <div className="flex items-center justify-center pb-8  h-80">
        ...
        <div className="absolute w-[60%] h-full border-x-[1px] px-4 rounded-lg border-x-orange-200">
          {userPost
              ...
              <div className="space-y-5" key={post.id}>
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">{favCount}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <svg
